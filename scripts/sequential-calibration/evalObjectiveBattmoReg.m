function [objValue, varargout] = evalObjectiveBattmoReg(pvec, objFunc, setup, parameters, varargin)
%
...
%}
    ...
    failure = false;
    %try
        objValues = objFunc(setupNew.model, states, setupNew.schedule, parameters,pvec);
        objValue  = sum(vertcat(objValues{:}))/opt.objScaling ;
    % catch
    %     objValue = NaN;
    %     failure = true;
    % end
        assert(all(isfinite([objValues{:}])), 'Objective function values are not finite');
        ...
        if failure
            ...
            switch opt.gradientMethod
              ...
              case 'AdjointAD'
                ...
                objh = @(tstep, model, state, computeStatePartial) objFunc(model, states, setupNew.schedule,parameters,pvec, ...
                ...
                if strcmp(opt.OptimizationSolver, 'unitboxbfgs')
                    for k = 1 : numel(nms)
                        scaledGradient{k} = parameters{k}.scaleGradient(gradient{k}, pval{k});
                    end
                elseif strcmp(opt.OptimizationSolver, 'unitboxlm')
                    for k = 1 : numel(nms)
                        scaledGradient{k} = parameters{k}.scaleGradient(gradient{k}, pval{k})';
                    end
                end
              ...
              case 'PerturbationADNUM'
                ...
                elseif ~iscell(pertsize) && isscalar(pertsize) % scalar value
                ...
                parfor iparam = 1 : numel(parameters)
                    ...
                    for i = 1 : np
                        ...
                        val(i) = evalObjectiveBattmoReg(pert_pvec, objFunc, setupNew, parameters, ...
                                                        'gradientMethod' , 'None'             , ...
